...
router.post("/push/notify", async (req, res) => {
  ...
  try {
    ...
    if (convError || !conversation) {
      req.log.error({ convError, conversationId }, "Conversation not found or error");
    ...
    // Authorization check: Is the sender a participant?
    // @ts-ignore
    ...
    // Authorization check: Are all recipients participants?
    const unauthorizedRecipients = recipientUserIds.filter(id => !participants.includes(id));
    if (unauthorizedRecipients.length > 0) {
      req.log.warn({ unauthorizedRecipients, conversationId }, "Attempted to notify non-participants");
      res.status(403).json({ error: "Forbidden: Some recipients are not participants in this conversation" });
      return;
    }
