...
router.post("/push/notify", async (req, res) => {
  ...
  try {
    ...
    // Authorization check: sender and recipients must be part of the conversation
    ...
    if (convErr || !conv) {
      req.log.error({ convErr, conversationId }, "Conversation not found for push notification");
    ...
    if (unauthorizedRecipients.length > 0) {
      res.status(403).json({
        error: "Unauthorized: One or more recipients are not participants in this conversation",
      });
      return;
    }
