...
router.post("/push/notify", async (req, res) => {
  ...
  try {
    ...
    // Verify sender and recipients are in the conversation
    ...
    if (convError || !conv) {
      req.log.error({ convError, conversationId }, "Conversation not found");
    ...
    if (unauthorizedRecipients.length > 0) {
      res.status(403).json({ error: "Unauthorized: Some recipients are not in this conversation" });
      return;
    }
