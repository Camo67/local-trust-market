...
router.post("/push/notify", async (req, res) => {
  ...
  try {
    ...
    // 1. Authorization check: Is sender a participant in the conversation?
    ...
    // 2. Authorization check: Are all recipients participants in the conversation?
    const invalidRecipients = recipientUserIds.filter(id => !participants.includes(id));
