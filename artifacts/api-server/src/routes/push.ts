...
router.post("/push/notify", async (req, res) => {
  ...
  try {
    ...
    // Authorization check: Verify sender and recipients are participants in the conversation
    ...
    if (convError || !conversation) {
      req.log.warn({ conversationId, convError }, "Conversation not found for push notify");
    ...
    if (!participants.has(req.user.id)) {
      // @ts-ignore
      req.log.warn({ userId: req.user.id, conversationId }, "Unauthorized sender for push notify");
      res.status(403).json({ error: "Unauthorized: You are not a participant in this conversation" });
      return;
    }

    if (!recipientUserIds.every(id => participants.has(id))) {
      req.log.warn({ recipientUserIds, conversationId }, "Unauthorized recipients for push notify");
