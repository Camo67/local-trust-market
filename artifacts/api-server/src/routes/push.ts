...
router.post("/push/notify", async (req, res) => {
  ...
  if (!Array.isArray(recipientUserIds) || !recipientUserIds.length || !body || !conversationId) {
    res.status(400).json({ error: "recipientUserIds (array), body, and conversationId are required" });
    return;
  }
  ...
  try {
    ...
    // Authorization check: Verify that sender and recipients are participants in the conversation
    ...
    if (convErr || !conv) {
      req.log.error({ convErr, conversationId }, "Conversation not found for push authorization");
    ...
    const allRecipientsAuthorized = recipientUserIds.every(id => participants.includes(id));
    if (!allRecipientsAuthorized) {
      res.status(403).json({ error: "Unauthorized: One or more recipients are not participants in this conversation" });
      return;
    }
