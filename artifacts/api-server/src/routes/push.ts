...
router.post("/push/notify", async (req, res) => {
  ...
  if (!Array.isArray(recipientUserIds) || !recipientUserIds.length || !body) {
    res.status(400).json({ error: "recipientUserIds (array) and body required" });
    return;
  }
  ...
  try {
    ...
    // Authorization check: Verify that the sender is a participant in the conversation
    // and that all recipients are also participants in the same conversation.
    ...
    if (convError || !conversation) {
      req.log.error({ convError, conversationId }, "Conversation not found for push authorization");
