...
router.post("/push/notify", async (req, res) => {
  ...
  try {
    ...
    // Authorization: verify that sender and recipients are participants in the conversation
    ...
    if (convError || !conversation) {
      ...
      res.status(403).json({ error: "Unauthorized: Invalid conversation" });
      return;
    }
    ...
    if (!recipientUserIds.every((id) => participants.has(id))) {
      res.status(403).json({ error: "Unauthorized: One or more recipients are not participants" });
      return;
    }
