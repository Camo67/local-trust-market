...
router.post("/push/notify", async (req, res) => {
  ...
  try {
    ...
    // Authorization check: Verify sender and recipients are participants of the conversation
    ...
    if (convError || !conversation) {
      // @ts-ignore
      req.log.error({ convError, conversationId }, "Conversation not found or error fetching it");
    ...
    if (unauthorizedRecipients.length > 0) {
      res.status(403).json({ error: "Unauthorized: One or more recipients are not participants of this conversation" });
      return;
    }
