...
## 2026-05-11 - [Generic Error Messages]
**Vulnerability:** Multiple API endpoints and serverless functions were returning raw `error.message` to clients, potentially leaking internal implementation details, database schema information, or third-party API errors (e.g., Resend).
**Learning:** Developers often use `error.message` for convenience during debugging, but it violates the principle of failing securely by exposing sensitive system information.
**Prevention:** Always catch errors and return generic messages (e.g., "Internal server error") to the client, while logging the detailed error server-side for internal debugging.
