...
const ListingCard = ({ listing }: { listing: ListingWithSeller }) => {
  ...
  const initials = listing.seller?.display_name
    ?.split(" ")
    .map((n) => n[0])
    .join("")
    .toUpperCase()
  ...
  return (
    ...
    >
      {listing.image_url ? (
        <img src={listing.image_url} alt={listing.title} className="w-full h-36 object-cover" loading="lazy" />
      ) : (
      ...
      <div className="p-3">
        ...
        <p className="text-sm text-muted-foreground mt-0.5">
          {listing.seller?.display_name || "Unknown"} · {listing.location}
        </p>
